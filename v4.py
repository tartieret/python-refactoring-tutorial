...
def load_data(data: APIData) -> bool:
    ...
    logger.info(f"Sending batch for category_id = {data.category_id} with {len(data.data)} records to API")
    ...
    try:
        ...
        if response.status_code >= 200 and response.status_code < 300:
            logger.info(f"Batch {data.category_id} - API request successful: Status Code {response.status_code}")
            return True
        else:
            logger.error(f"Batch {data.category_id} - API request failed: Status Code {response.status_code}, Response: {response.text}")
            return False
