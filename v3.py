...
def transform_data(category_id: int, records: List[Tuple]) -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: A dictionary with category_id and data array containing the transformed records.
    """
    logger.info(f"Transforming {len(records)} records for category_id = {category_id}")

    transformed = {"category_id": category_id, "data": []}
...
def load_data(data: Dict[str, Any]) -> bool:
    ...
    Args:
        data (Dict[str, Any]): A dictionary containing the transformed data with category_id and data array.
    ...
    logger.info(
        f"Sending batch for category_id = {category_id} with {record_count} records to API"
    )
    ...
    try:
        ...
        if response.status_code >= 200 and response.status_code < 300:
            logger.info(
                f"Batch {category_id} - API request successful: Status Code {response.status_code}"
            )
            return True
        else:
            logger.error(
                f"Batch {category_id} - API request failed: Status Code {response.status_code}, Response: {response.text}"
            )
            return False
